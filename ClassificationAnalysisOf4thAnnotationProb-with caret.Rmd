...
#read data
```{r}
mood.industry.data=readRDS("mood.industry.data.with.prob.4th.rds")
wordfrequency.matrix.regcan=readRDS("wordfrequency.matrix.regcan.norm.4th.rds")

ngramfrequency.matrix.regcan=readRDS("ngramfrequency.matrix.regcan.norm.4th.rds")

reduced.matrix.context.rias=readRDS("reduced.matrix.context.rias.400.4th.rds")
reduced.matrix.context.rias=  as.matrix(reduced.matrix.context.rias$u %*% solve(diag((reduced.matrix.context.rias$d)))) %>%  set_colnames(., reduced.matrix.context.rias$feature.names)

svd.of.regcan.from.irba = readRDS("svd.of.regcan.from.irba.200.4th.rds")
svd.of.regcan.from.irba=as.matrix(svd.of.regcan.from.irba$u %*% solve(diag((svd.of.regcan.from.irba$d)))) %>%  set_colnames(., svd.of.regcan.from.irba$feature.names)
...
#Combine features
##keep relevant features from data
```{r}
data.classification=mood.industry.data[,c("sponsor.feature", "type.feature", "act.feature", "label_of_class", "training.set", "is.not.consultation.prob","statement.advocacy.prob", "statement.institution.prob", "statement.industry.prob")]
```

##add word-matrix frequency
```{r}
data.classification=cbind(data.classification, as.matrix(wordfrequency.matrix.regcan)%>%set_colnames(., colnames(wordfrequency.matrix.regcan)))
```

##add n-gram
```{r}
data.classification=cbind(data.classification, as.matrix(ngramfrequency.matrix.regcan)%>%set_colnames(., colnames(ngramfrequency.matrix.regcan)))
```
...
##add latent models
```{r}
data.classification=cbind(data.classification, svd.of.regcan.from.irba%>%set_colnames(., colnames(svd.of.regcan.from.irba)))
```
...
##subset of id train
subset a training set. 2/3 for training
```{r}
seed=123
...
###set features
keep all features except class labels other than is.not.consultation annotation
```{r}
#
# data.classification.train.target = data.classification.train[,-c("label_of_class", "statement.advocacy.prob", "statement.institution.prob", "statement.industry.prob")]

data.classification.train.target = data.classification.train %>% subset(., select = -c(label_of_class, statement.advocacy.prob, statement.institution.prob, statement.industry.prob))
...
rf.notConsult.model=ranger::ranger(dependent.variable.name = "targeted.class", data=data.classification.train.target[id.sub.train,], num.trees=40000, num.threads=6, classification=T, probability = T)
...
saveRDS(rocCurve.of.annot, "rocCurve.NoConsultation.rds")
...
<!-- ##is.not.consultation without prob -->
<!-- ###train on sub training set -->
<!-- ```{r} -->

<!-- rf.notConsult.model=ranger::ranger(dependent.variable.name = "targeted.class", data=data.classification.train.target[id.sub.train,], num.trees=20000, num.threads=6, classification=T, importance = "impurity", probability = F) -->
...
<!-- ###evaluation on confusion matrix -->
<!-- ```{r} -->
<!-- eval.of.annotation<-caret::confusionMatrix(data=predicted, reference=observed, mode="everything", positive="TRUE") -->
...
data.classification.train.target = data.classification.train %>% subset(., select = -c(label_of_class, statement.advocacy.prob, is.not.consultation.prob, statement.industry.prob))
...
rf.institution.model=ranger::ranger(dependent.variable.name = "targeted.class", data=data.classification.train.target[id.sub.train,], num.trees=40000, num.threads=6, classification=T, probability = T)
...
saveRDS(rocCurve.of.annot, "rocCurve.institution.rds")
...
data.classification.train.target = data.classification.train %>% subset(., select = -c(label_of_class, statement.institution.prob, is.not.consultation.prob, statement.industry.prob))
...
rf.advocacy.model=ranger::ranger(dependent.variable.name = "targeted.class", data=data.classification.train.target[id.sub.train,], num.trees=40000, num.threads=6, classification=T, probability = T)
...
saveRDS(rocCurve.of.annot, "rocCurve.advocacy.rds")
...
data.classification.train.target = data.classification.train %>% subset(., select = -c(label_of_class, statement.institution.prob, is.not.consultation.prob, statement.advocacy.prob))
...
rf.industry.model=ranger::ranger(dependent.variable.name = "targeted.class", data=data.classification.train.target[id.sub.train,], num.trees=40000, num.threads=6, classification=T, probability = T)
...
saveRDS(rocCurve.of.annot, "rocCurve.industry.rds")
...
changer la sélection des feature pour les prédiction sur le test set
...
###set features
```{r}
# data.classification.target = data.classification
# 
# data.classification.target$targeted.class=data.classification.target$is.not.consultation.prob
# data.classification.target$is.not.consultation.prob=NULL


data.classification.target = data.classification %>% subset(., select = -c(label_of_class, statement.advocacy.prob, statement.institution.prob, statement.industry.prob))
...
```{r}
my.threshold=0.1 #at least greater than 0
data.classification.target$targeted.class=(data.classification.target$targeted.class>my.threshold) %>%as.factor(.)
```

###train on all training set
```{r}

rf.notConsult.model=ranger::ranger(dependent.variable.name = "targeted.class", data=data.classification.target[id.train,], num.trees=40000, num.threads=6, classification=T, probability = T)
...
rf.notConsul.pred=predict(rf.notConsult.model, data =  data.classification.target[, names(data.classification.target)!="targeted.class"])

pred.of.noConsult.prob=rf.notConsul.pred$predictions[,"TRUE"]
...
###set features
```{r}
# data.classification.target = data.classification[,-c(4:7)]
# 
# data.classification.target$targeted.class=data.classification.target$statement.industry.prob
# data.classification.target$statement.industry.prob=NULL


data.classification.target = data.classification %>% subset(., select = -c(label_of_class, statement.institution.prob, is.not.consultation.prob, statement.advocacy.prob))
...
rf.industry.model=ranger::ranger(dependent.variable.name = "targeted.class", data=data.classification.target[id.train,], num.trees=40000, num.threads=6, classification=T, probability = T)
...
rf.industry.pred=predict(rf.industry.model, data =  data.classification.target[,names(data.classification.target)!="targeted.class"])

pred.of.industry.prob=rf.industry.pred$predictions[,"TRUE"]
...
##Save results
```{r}
mood.industry.data.with.pred.on.testset=cbind(mood.industry.data, pred.of.noConsult.prob, pred.of.industry.prob)
saveRDS(mood.industry.data.with.pred.on.testset, "mood.industry.data.with.pred.on.testset.rds")

write.table(mood.industry.data.with.pred.on.testset, file="mood.industry.data.with.pred.on.testset.csv", col.names = T, fileEncoding = "utf8", quote=T, sep=",")
```
...
###set features
```{r}
data.classification.target = data.classification[,-c(4,6:8)]
...
```{r}
set.seed(825)
gbmFit1 <- train(targeted.class ~ ., data = data.classification.target[id.train,], 
                 method = "gbm", 
